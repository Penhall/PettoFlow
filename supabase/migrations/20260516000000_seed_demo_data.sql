...
begin
  ...
  if v_col_id is null then
    raise notice 'Tenant "central" has no kanban columns - skipping task seed';
  end if;
  ...
  if v_col_id is not null and (select count(*) from public.tasks where tenant_id = v_tenant_id) = 0 then
    select exists (
      select 1 from information_schema.columns
