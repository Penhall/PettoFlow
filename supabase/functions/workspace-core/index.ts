...
function pickFields(value, allowedFields) {
  const source = toRecord(value);
  return Object.fromEntries(allowedFields.filter((field)=>hasOwn(source, field)).map((field)=>[
      field,
      source[field]
    ]));
}
...
function parsePageNumber(value, fallback, label) {
  if (value === null || value === '') return {
    value: fallback,
    error: null
  };
...
function parsePageSize(value, fallback, label) {
  if (value === null || value === '') return {
    value: fallback,
    error: null
  };
...
function scopeTenantQuery(query, tenantId) {
  return query.eq('tenant_id', tenantId);
}
function injectTenantId(body, tenantId) {
  return {
    ...body,
    tenant_id: tenantId
  };
}
async function assertTableCreationWithinLimit(sb, tenantId, tableName) {
  ...
  let query = scopeTenantQuery(sb.from(tableName).select('id', {
    count: 'exact',
    head: true
  }), tenantId);
  if (tableName === 'tasks') {
...
const CATEGORY_GROUP_FIELDS = [
  'name',
  'sort_order',
  'is_income'
];
const FIN_CATEGORY_FIELDS = [
  'name',
  'group_id',
  'sort_order',
  'hidden'
...
const RECEIVABLE_CREATE_FIELDS = [
  'task_id',
  'activity_id',
  'amount',
  'target_account_id',
...
const RECEIVABLE_UPDATE_FIELDS = [
  'amount',
  'target_account_id',
  'status',
  'due_date',
  ...
  try {
    if (req.method === 'GET' && resource === 'bootstrap') {
      const [tasksResult, teamResult, clientsResult, columnsResult] = await Promise.all([
        scopeTenantQuery(sb.from('tasks').select('*').is('archived_at', null), tenantId).order('created_at', {
          ascending: false
        }),
      ...
      if (tasksResult.error) return json(req, {
        error: tasksResult.error.message
      }, 500);
      if (teamResult.error) return json(req, {
        error: teamResult.error.message
      }, 500);
      if (clientsResult.error) return json(req, {
        error: clientsResult.error.message
      }, 500);
      if (columnsResult.error) return json(req, {
    ...
    if (req.method === 'GET' && resource === 'tasks' && resourceId === 'archived') {
      ...
      if (parsedPage.error) return json(req, {
        error: parsedPage.error
      }, 400);
      const parsedPageSize = parsePageSize(url.searchParams.get('pageSize'), 50, 'pageSize');
      if (parsedPageSize.error) return json(req, {
        error: parsedPageSize.error
      }, 400);
      ...
      let query = scopeTenantQuery(sb.from('tasks').select('*', {
        count: 'exact'
      }).not('archived_at', 'is', null).order('archived_at', {
        ascending: false
      }).range(from, to), tenantId);
      if (category) query = query.eq('category', category);
      if (tag) query = query.contains('tags', [
        tag
      ]);
    ...
    if (req.method === 'PATCH' && resource === 'tasks' && resourceId) {
      ...
      if (!id) return json(req, {
        error: 'Task id invalido'
      }, 400);
    ...
    if (req.method === 'POST' && resource === 'tasks' && resourceId && action === 'archive') {
      ...
      let query = sb.from('tasks').update({
    ...
    if (req.method === 'POST' && resource === 'team' && !resourceId) {
      ...
      const { data, error } = await sb.from('team').insert([
        body
      ]).select().single();
      if (error) return json(req, {
        error: error.message
    ...
    if (req.method === 'GET' && resource === 'activities' && !resourceId) {
      const { data, error } = await scopeTenantQuery(sb.from('activities').select('*'), tenantId).order('created_at', {
        ascending: false
      });
      if (error) return json(req, {
        error: error.message
    ...
    if (req.method === 'POST' && resource === 'activity-templates' && !resourceId) {
      ...
      const { data, error } = await sb.from('activity_templates').insert([
        body
      ]).select().single();
      if (error) return json(req, {
        error: error.message
    ...
    if (req.method === 'GET' && resource === 'accounts' && !resourceId) {
      const { data, error } = await scopeTenantQuery(sb.from('accounts').select('*'), tenantId).order('sort_order', {
        ascending: true
      });
      if (error) return json(req, {
        error: error.message
    ...
    if (req.method === 'GET' && resource === 'payees' && !resourceId) {
      const { data, error } = await scopeTenantQuery(sb.from('payees').select('*'), tenantId).order('name', {
        ascending: true
      });
      if (error) return json(req, {
        error: error.message
    ...
    if (req.method === 'POST' && resource === 'payees' && !resourceId) {
      ...
      const { data, error } = await sb.from('payees').insert([
        body
      ]).select().single();
      if (error) return json(req, {
        error: error.message
    ...
    if (req.method === 'GET' && resource === 'fin-rules' && !resourceId) {
      const { data, error } = await scopeTenantQuery(sb.from('fin_rules').select('*'), tenantId).order('priority', {
        ascending: true
      });
      if (error) return json(req, {
        error: error.message
    ...
    if (req.method === 'POST' && resource === 'fin-rules' && !resourceId) {
      ...
      const { data, error } = await sb.from('fin_rules').insert([
        body
      ]).select().single();
      if (error) return json(req, {
        error: error.message
    ...
    if (req.method === 'GET' && resource === 'fin-categories' && !resourceId) {
      ...
      if (groupsRes.error) return json(req, {
        error: groupsRes.error.message
      }, 500);
      if (catsRes.error) return json(req, {
        error: catsRes.error.message
      }, 500);
      return json(req, {
    ...
    if (req.method === 'POST' && resource === 'fin-categories' && resourceId === 'groups') {
      ...
      const { data, error } = await sb.from('category_groups').insert([
        body
      ]).select().single();
      if (error) return json(req, {
        error: error.message
    ...
    if (req.method === 'POST' && resource === 'fin-categories' && resourceId === 'items') {
      ...
      const { data, error } = await sb.from('fin_categories').insert([
        body
      ]).select().single();
      if (error) return json(req, {
        error: error.message
    ...
    if (req.method === 'GET' && resource === 'transactions' && !resourceId) {
      ...
      if (relatedId.error) return json(req, {
        error: relatedId.error
      }, 400);
      const dateFrom = url.searchParams.get('dateFrom');
      const dateTo = url.searchParams.get('dateTo');
      ...
      if (onlyNegative) {
        if (onlyNegative !== 'true') return json(req, {
          error: 'onlyNegative invalido'
        }, 400);
        query = query.lt('amount', 0);
      }
      if (relatedType && relatedId.value) {
        query = query.contains('related_to', [
          {
            type: relatedType,
            id: relatedId.value
          }
        ...
        return json(req, {
          error: 'relatedType e relatedId sao obrigatorios juntos'
        }, 400);
      }
    ...
    if (req.method === 'POST' && resource === 'transactions' && !resourceId) {
      ...
      ctx.log('info', 'transaction_created', {
        tenant_id: tenantId,
        user_id: auth.user.id,
    ...
    if (req.method === 'GET' && resource === 'receivables' && !resourceId) {
      const { data, error } = await scopeTenantQuery(sb.from('receivables').select(`
          *,
    ...
    if (req.method === 'POST' && resource === 'receivables' && !resourceId) {
      ...
      const { data, error } = await sb.from('receivables').insert([
        body
      ]).select().single();
      if (error) return json(req, {
        error: error.message
    ...
    if (req.method === 'GET' && resource === 'interaction-logs' && !resourceId) {
      ...
      if (clientId.error || !clientId.value) {
        return json(req, {
          error: clientId.error ?? 'clientId obrigatorio'
        }, 400);
      }
      const { data, error } = await scopeTenantQuery(sb.from('interaction_logs').select('*').eq('client_id', clientId.value).order('created_at', {
        ascending: false
      }), tenantId);
      if (error) return json(req, {
        error: error.message
    ...
    if (req.method === 'POST' && resource === 'interaction-logs' && !resourceId) {
      ...
      const { data, error } = await sb.from('interaction_logs').insert([
        body
      ]).select().single();
      if (error) return json(req, {
        error: error.message
