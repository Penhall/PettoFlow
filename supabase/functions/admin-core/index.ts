...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (request.method === 'GET' && resource === 'billing') {
      ...
      if (subsResult.error) return ctx.fail(500, 'admin_billing_subs_failed', subsResult.error.message)
      if (eventsResult.error) return ctx.fail(500, 'admin_billing_events_failed', eventsResult.error.message)

      const subs = subsResult.data ?? []
      const activeSubs = subs.filter(s => s.status === 'active')
      const inactiveSubs = subs.filter(s => s.status === 'inactive' || s.status === 'canceled')

      const mrrTotal = activeSubs.reduce((sum, s) => sum + Number((s.plan as any)?.price_monthly ?? 0), 0)
      const churnedMrr = inactiveSubs.reduce((sum, s) => sum + Number((s.plan as any)?.price_monthly ?? 0), 0)

      const byPlanMap = new Map<string, { plan_name: string; active: number; mrr: number; tenants: number }>()
      for (const s of subs) {
        const planName = (s.plan as any)?.name ?? 'free'
        const price = Number((s.plan as any)?.price_monthly ?? 0)
        const entry = byPlanMap.get(planName) ?? { plan_name: planName, active: 0, mrr: 0, tenants: 0 }
        entry.tenants += 1
        if (s.status === 'active') {
          entry.active += 1
          entry.mrr += price
        }
        byPlanMap.set(planName, entry)
      }

      const now = new Date()
      const subscriptionsOverview = subs.slice(0, 50).map(s => {
        const end = s.current_period_end ? new Date(s.current_period_end) : null
        const daysUntilRenewal = end ? Math.ceil((end.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)) : null
        return {
          tenant_id: s.tenant_id,
          tenant_name: (s.tenant as any)?.name ?? null,
          plan_name: (s.plan as any)?.name ?? 'free',
          status: s.status,
      ...
      const recentEvents = (eventsResult.data ?? []).map(e => ({
        id: e.id,
        tenant_id: e.tenant_id,
        tenant_name: (e.tenant as any)?.name ?? null,
      ...
      return ctx.ok({
        summary: {
          ...
          avg_revenue_per_tenant: activeSubs.length > 0 ? mrrTotal / activeSubs.length : 0,
        },
